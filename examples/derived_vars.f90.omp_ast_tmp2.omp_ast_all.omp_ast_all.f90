MODULE custom_type_mod
  ...
  END SUBROUTINE init_obj
    ...
    !$omp parallel do private(i) schedule(static)
      ...
      DO i = 1, n
      apf_tmp_data_arr(i) = obj % prod_arr(i) + obj % sum_val
      ! 流依赖（obj → temp_obj）
    END DO
    apf_tmp_data_arr % i = apf_tmp_i_2
    DEALLOCATE(apf_tmp_i_2)
    ...
    REAL, ALLOCATABLE :: apf_tmp_data_arr_2(:)
    ALLOCATE(apf_tmp_data_arr_2(SIZE(obj % data_arr)))
    apf_tmp_data_arr_2 = obj % data_arr
    INTEGER, ALLOCATABLE :: apf_tmp_i(:)
    ALLOCATE(apf_tmp_i(SIZE(apf_tmp_data_arr_2 % i)))
    apf_tmp_i = apf_tmp_data_arr_2 % i

      ! 5. 自依赖赋值：当前对象数组元素依赖自身前序元素（含步长2的跨迭代依赖）
      DO i = 3, n, 2
      ! 步长为2：i=3,5,7...
      apf_tmp_data_arr_2(i) = apf_tmp_data_arr_2(i - 2) * 0.5
      ! 流依赖（i-2 → i）
    END DO
    apf_tmp_data_arr_2 % i = apf_tmp_i
    DEALLOCATE(apf_tmp_i)
    obj % data_arr = apf_tmp_data_arr_2
    DEALLOCATE(apf_tmp_data_arr_2)
    ...
    !$omp parallel do private(i) schedule(static)
      ...
      DO i = 1, n
      apf_tmp_prod_arr(i) = 2.0 * obj % data_arr(i)
