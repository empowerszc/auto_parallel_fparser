...
!$omp parallel do private(i) reduction(+:apf_tmp_sum_val) schedule(static)
        do i = 1, n
            if (apf_tmp_data_arr(i) > 5.0) then  ! 条件筛选
                apf_tmp_sum_val = apf_tmp_sum_val + apf_tmp_data_arr(i)  ! 归约：输出依赖（i-1 → i）
            end if
        end do
!$omp end parallel do
obj % data_arr = apf_tmp_data_arr
...
apf_tmp_data_arr = obj % data_arr
        do i = 1, n
            if (i == 1) then
                apf_tmp_prod_arr(i) = apf_tmp_data_arr(i)  ! 初始值
            else
                apf_tmp_prod_arr(i) = apf_tmp_prod_arr(i-1) * apf_tmp_data_arr(i)  ! 流依赖（i-1 → i）
            end if
        end do
...
!$omp parallel do private(i) schedule(static)
        do i = 1, n
            if (apf_tmp_data_arr(i) > apf_tmp_max_val) then
                apf_tmp_max_val = apf_tmp_data_arr(i)  ! 归约：输出依赖（i-1 → i，条件性）
            end if
        end do
!$omp end parallel do
obj % data_arr = apf_tmp_data_arr
...
!$omp parallel do private(i) schedule(static)
        do i = 1, n
            apf_tmp_data_arr(i) = apf_tmp_prod_arr(i) + apf_tmp_sum_val  ! 流依赖（obj → temp_obj）
        end do
!$omp end parallel do
...
apf_tmp_data_arr = obj % data_arr
        do i = 3, n, 2  ! 步长为2：i=3,5,7...
            apf_tmp_data_arr(i) = apf_tmp_data_arr(i-2) * 0.5  ! 流依赖（i-2 → i）
        end do
obj % data_arr = apf_tmp_data_arr
...
!$omp parallel do private(i) schedule(static)
        do i = 1, n
            apf_tmp_prod_arr(i) = 2.0 * apf_tmp_data_arr(i)  ! 输出依赖（与步骤2的prod_arr(i)冲突）
        end do
!$omp end parallel do
obj % prod_arr = apf_tmp_prod_arr
